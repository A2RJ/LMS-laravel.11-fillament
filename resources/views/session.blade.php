...
@section('content')
<!-- Breadcrumb -->
<nav class="max-w-screen-lg lg:max-w-screen-xl mx-auto flex pl-4 py-3 text-slate-700 rounded-lg dark:bg-slate-800 dark:border-slate-700" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="ms-1 text-sm font-medium text-slate-500 md:ms-2 dark:text-slate-400">{{ $course->title }}</span>
            </div>
        </li>
    </ol>
</nav>
...
<div class="max-w-screen-lg lg:max-w-screen-xl mx-auto bg-white border border-slate-300 rounded-lg grid grid-cols-12">
    <div class="col-span-9 border-r border-dashed border-slate-300 py-1 px-4">
        ...
        <div class="mb-4 border-b-2 pb-4 border-dashed border-transparent inline-flex space-x-4 overflow-x-scroll max-w-full">
            <a href="{{ route('course.id', $course->id) }}" class="whitespace-nowrap inline-flex rounded-lg py-2 px-3 text-sm font-medium text-green-700 bg-green-50 border border-green-300 transition-all duration-200 ease-in-out hover:bg-green-500 hover:text-white ">
                Introduction
            </a>
        ...
        <div class="flex justify-between mx-auto mt-3 mb-6">
            ...
            <a href="{{ route('course.id', ['course' => $course->id]) }}">
                <button class="relative group flex items-center justify-center px-3 py-1 border border-blue-500 overflow-hidden rounded-lg bg-blue-500 text-white">
                    ...
                    <div class="absolute inset-0 bg-white transform scale-x-0 origin-left transition-transform duration-300 group-hover:scale-x-100">
                    </div>
                </button>
            </a>
            @else
    ...
    <div class="col-span-3 pl-1">
        <div class="sticky top-[70px] max-h-screen overflow-y-scroll p-2 pb-24 border border-transparent rounded-lg space-y-2 bg-white w-full mt-2">
            ...
            <div role="alert" class="mb-4 rounded border-s-4 border-red-500 bg-red-50 p-4">
                ...
                <form action="{{ route('sessions.attendance', ['course' => $course->id, 'session' => $data['id']]) }}" method="POST">
                    @csrf
                    <button type="submit" class="flex justify-between items-center space-x-2 mt-4 text-green-700 border border-green-300 rounded-lg bg-green-50 font-medium text-sm py-2 px-4">
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                    <button type="button" class="mt-3 border border-transparent inline-flex w-full justify-center rounded-md bg-blue-500 px-3 py-2 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-white hover:text-black hover:border sm:mt-0 sm:w-auto" id="cancel-save">Cancel</button>
                </div>
            </div>
        </div>
    </div>
