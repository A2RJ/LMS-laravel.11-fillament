    ...
    <div class="col-span-9 border-r border-dashed border-slate-300 py-1 px-4">
        ...
        <div
            ...
            @foreach ($course->sessions as $index => $session)
                <a href="{{ route('course.id', ['course' => $course->id, 'session' => $session->id]) }}"
        ...
        <div
            ...
            @else
                <div role="alert" class="mb-4 rounded border-s-4 border-red-500 bg-red-50 p-4">
                    ...
                    <form action="{{ route('sessions.attendance', ['session' => request('page')]) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="flex justify-between items-center space-x-2 mt-4 text-green-700 border border-green-300 rounded-lg bg-green-50 font-medium text-sm py-2 px-4">
