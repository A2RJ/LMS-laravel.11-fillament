        ...
        <div class="p-10 pl-20 pt-14">
            ...
            <div class="flex flex-row space-x-4">
                <p class="flex items-center gap-1 text-sm leading-none text-white">
                    ...
                    <span>{{ count($this->record->sessions) }} Session</span>
                </p>
                <p class="flex items-center gap-1 text-sm leading-none text-white">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-white" width="32" height="32"
                        viewBox="0 0 48 48">
                    ...
                    <span>{{ count($this->record->users) }} Teachers</span>
                </p>
                ...
                <div class="overflow-x-auto rounded-lg border border-gray-200 mb-4">
                    <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                        <thead class="ltr:text-left rtl:text-right">
                            <tr>
                                <th colspan="4" class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">User:
                                    {{ $course['user'] }}
                                </th>
                            </tr>
                            <tr>
                                <th colspan="4" class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Course:
                                    <a href="{{ route('course.id', ['course' => $this->record->id]) }}"
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($course['session'] as $index => $session)
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                        @if ($session['attendance'])
                                            <input type="checkbox" class="h-5 w-5 text-green-600 border-gray-300 rounded" checked
                                                disabled>
                                        @else
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                        <a href="{{ route('session.id', ['course' => $this->record->id, 'page' => $index + 1]) }}"
                                            class="inline-flex items-center gap-2 rounded border border-indigo-600 bg-indigo-600 px-2 py-1 text-white hover:bg-transparent hover:text-indigo-600 focus:outline-none focus:ring active:text-indigo-500">
                                            <span class="text-sm font-medium"> Detail </span>
                                            <svg class="size-3 rtl:rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none"
                                ...
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                        {{ $session['pre_test_result'] ? 'Passed' : 'N/A' }}
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $session['pre_test_total_score'] }}
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                        <a href="{{ route('filament.admin.resources.sessions.check.user.test', ['record' => $session['pre_test_result_number']]) }}"
                                            class="inline-flex items-center gap-2 rounded border px-2 py-1 text-indigo-600 hover:bg-transparen focus:outline-none focus:ring active:bg-indigo-500">
                                            <span class="text-sm font-medium"> Detail </span>
                                            <svg class="size-3 rtl:rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                        {{ $session['post_test_result'] ? 'Passed' : 'N/A' }}
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                        {{ $session['post_test_total_score'] }}
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                        <a href="{{ route('filament.admin.resources.sessions.check.user.test', ['record' => $session['post_test_result_number']]) }}"
                                            class="inline-flex items-center gap-2 rounded border px-2 py-1 text-indigo-600 hover:bg-transparen focus:outline-none focus:ring active:bg-indigo-500">
                                            <span class="text-sm font-medium"> Detail </span>
                                            <svg class="size-3 rtl:rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
