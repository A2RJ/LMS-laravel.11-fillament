<x-filament-panels::page class="p-0 dark:bg-inherit m-0 min-h-screen">
    ...
    <div class="p-4 pb-20 bg-white rounded-md dark:bg-inherit border dark:border-slate-500">
        ...
        <div class="flex justify-start items-center gap-1 mt-2">
            <svg class="bg-blue-500 w-6 h-6 p-1 rounded-full text-white" xmlns="http://www.w3.org/2000/svg" fill="none"
        ...
        <div
            class="flex flex-col mt-2 lg:flex-row text-slate-500 dark:text-white font-normal gap-1 lg:gap-4 text-xs lg:text-sm">
                    ...
                    <li>
                        ...
                        @if ($question->answer_type == 'selected')
                            @if (!$question->answers->count())
                                <div
                                    class="h-14 inline-flex items-center justify-center gap-1 w-full p-4 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 dark:peer-checked:text-blue-500 peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
                                ...
                                <div class="grid grid-rows-1 gap-2">
                                    @foreach ($question->answers as $indexAnswer => $answer)
                                        <div class="row-span-1 flex items-start justify-start py-1 max-w-[80%]">
                                            <input id=" question{{ $index }}{{ $indexAnswer }}" type="radio"
                                                value="{{ $answer->answer }}" name="question{{ $index }}"
                                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-0 dark:bg-gray-700 dark:border-gray-600">
                                            <label for="question{{ $index }}{{ $indexAnswer }}"
                                                class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300 flex gap-1">{{ $options[$indexAnswer] }}.
                                                {!! $answer->answer !!}</label>
                                        </div>
                                    @endforeach
                                </div>
