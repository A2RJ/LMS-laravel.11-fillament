    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="ms-1 text-sm font-medium text-slate-500 md:ms-2 dark:text-slate-400">{{ $class->session->title }}</span>
            </div>
        </li>
    </ol>
</nav>
...
<div class="p-4 max-w-screen-lg mx-auto bg-white border border-slate-300 rounded-lg grid grid-cols-1 gap-4">
    <div class="flex justify-start items-center gap-2 mt-2">
        <div>
            <a href="{{ route('session.id', $class->course_id) }}?page={{ $class->session_id }}" class="group flex justify-between space-x-2 items-center bg-transparent p-2 rounded-md transition-colors duration-150 border border-transparent hover:bg-blue-500 hover:cursor-pointer">
                ...
                <div class="flow-root rounded-lg border border-slate-300 py-3 shadow-sm">
                    <dl class="-my-3 divide-y divide-slate-100 text-sm">
                        ...
                        <div class="grid grid-cols-1 gap-1 p-3 sm:grid-cols-3 sm:gap-4">
                            ...
                            <dd class="text-slate-700 sm:col-span-2">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-orange-300 text-orange-800 ">
                                    Still being checked
                                </span>
                            </dd>
                        </div>
        ...
        <div class="" data-index="{{ $index }}">
            ...
            <div class="ml-8">
                ...
                <label class="flex justify-start items-start space-x-2 space-y-4">
                    ...
                    @if ($option->is_true)
                    <svg class="w-6 h-6 mt-4" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 48 48">
                    ...
                    @else
                    <input type="radio" name="" value="" class="form-radio lg:h-5 lg:w-5 checked:bg-slate-500 text-slate-500 p-1 mt-5" disabled>
                    @endif
                    @endif
                ...
                <div class="border border-slate-300 rounded p-2 mt-2">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $test->score ? 'bg-green-100 text-green-500' : 'bg-orange-300 text-orange-800' }}">
